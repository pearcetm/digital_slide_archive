...
def adjust_ids(user_name):  # noqa
    ...
    if host_uid != user_uid or host_gid != user_gid:
        os.system('rm -rf /home/%s/.ansible' % user_name)
...
def set_hosts():
    ...
    if os.environ.get('HOST_MONGO') == 'true':
        if 'mongodb' not in [line.split()[-1] for line in hosts]:
            ...
            changed = True
    if os.environ.get('HOST_MEMCACHED') == 'true':
        if 'memcached' not in [line.split()[-1] for line in hosts]:
            hosts.append('%s memcached' % hostip)
            changed = True
